...
class ProjectCreateView(ProjectPermissionMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'Projeto "{self.object.name}" criado com sucesso.')
        return response
...
class ProjectDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        user = self.request.user
        context['can_manage'] = user.is_admin or TeamMembership.objects.filter(
...
class ProjectEditView(ProjectPermissionMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        messages.success(self.request, f'Projeto "{self.object.name}" atualizado com sucesso.')
        return response
