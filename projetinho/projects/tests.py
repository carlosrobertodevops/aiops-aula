...
class ProjectPermissionTests(TestCase):
    def setUp(self):
        ...
        TeamMembership.objects.create(team=self.team, user=self.member, is_manager=False)
...
class ProjectVisibilityTests(TestCase):
    ...
    def test_member_can_see_project_in_their_team(self):
        self.client.login(email='membro@e.com', password='Membro123')
        response = self.client.get(reverse('projects:project_detail', args=[self.project_a.pk]))
        self.assertEqual(response.status_code, 200)

    def test_member_cannot_see_project_in_other_team(self):
        self.client.login(email='membro@e.com', password='Membro123')
        response = self.client.get(reverse('projects:project_detail', args=[self.project_b.pk]))
...
class ProjectArchiveTests(TestCase):
    ...
    def test_reactivate_project(self):
        project = Project.objects.create(name='P1', team=self.team, status=Project.STATUS_ARCHIVED)
        self.client.post(reverse('projects:project_archive', args=[project.pk]))
        project.refresh_from_db()
        self.assertEqual(project.status, Project.STATUS_ACTIVE)

    def test_archived_projects_hidden_by_default(self):
        Project.objects.create(name='Active', team=self.team, status=Project.STATUS_ACTIVE)
        Project.objects.create(name='Archived', team=self.team, status=Project.STATUS_ARCHIVED)
        response = self.client.get(reverse('projects:project_list_by_team', args=[self.team.pk]))
    ...
    def test_archived_projects_visible_with_filter(self):
        ...
        response = self.client.get(
            reverse('projects:project_list_by_team', args=[self.team.pk]) + '?archived=1'
        )
